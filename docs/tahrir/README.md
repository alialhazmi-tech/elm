...
مراجعة الأمان والجودة 13 سبتمبر: كلمات مرور الطاقم الجديدة 15 محرفًا على الأقل مع حظر محلي لقيم شائعة، وnonce لسكربتات اللوحة. معاينة SEO إرشادية وتوليد رابط وصفي للمسودة مع حفظ رابط المنشور. التفاصيل والتفعيل في `docs/audit/remediation-2026-09-13.md`.

- تطبيع كلمات بحث المحرر مشترك مع البحث العام في `lib/content/search-normalize.ts`؛ إصلاح فهرس البحث العام في الترحيل 0017 لا يغيّر ترتيب بحث المواد أو حقوله.


## الموجز وبحث المواد — سبتمبر 2026

- الموجز حتى 500 حرف بحسب كثافة المادة، وعادةً جملتان أو ثلاث تغطي النتيجة وأهم تفسير أو أثر والقيد الضروري. تصل المادة كاملة للنموذج في التوليد المنفرد والملحقات والتحرير الشامل؛ لا اقتطاع للمخرجات، وتصحيح واحد عند الرفض مع احتساب الاستدعاءين. يبقى اقتراحًا يراجعه المحرر قبل التطبيق. حد الحقل موحّد في الويب وتطبيق iOS عند تحديثه؛ وصف SEO وملخص القارئ ذي الثلاث نقاط مستقلان.
- بحث «المواد» يشمل العنوان والموجز والكلمات المفتاحية والمتن، ويطابق جميع كلمات الاستعلام بأي ترتيب بعد توحيد الهمزات والتشكيل والتطويل والأرقام العربية والفارسية. تُهَرَّب محارف `%` و`_` و`\` لتبقى حرفية؛ لا تتحول إلى أنماط مفتوحة.
- الأولوية للعنوان المطابق، ثم العبارة في العنوان، ثم الكلمات في العنوان، ثم الحقول الوصفية، ثم المتن؛ الأحدث يفصل بين النتائج المتساوية. بلا بحث يبقى الترتيب الزمني كما هو. تستخدم قائمة الويب وواجهة التطبيق الشروط والترتيب نفسيهما، مع فلاتر الحالة والسلسلة والعدد والترقيم.
- الترحيل `0016_editor_content_search.sql` يضيف دالة تطبيع ونص بحث مخزّن وفهرس GIN يشمل الحالات كلها ويتحدث تلقائيًا مع الحفظ والاستيراد. لا يعتمد على مهمة إعادة فهرسة دورية. الاختبار `tests/editor-search.integration.mjs` يتحقق من النتائج والتحديثات واستخدام الفهرس على قاعدة محلية معزولة.
- التحقق المحلي: lint وTypeScript وبناء الويب و366 اختبار وحدة وميزانية JavaScript وجميع اختبارات التكامل ناجحة، وكذلك بناء iOS للمحاكي. من واجهة المواد أعاد البحث «السعودية 2030» المادة التي تحتوي «السُّعوديـة عام ٢٠٣٠» في متنها، وقدّم العنوان المطابق في بحث «الذكاء الاصطناعي». اختبار مخرجات التوليد الحقيقية لم يكتمل لغياب مفتاح Anthropic محليًا؛ نجاح اختبارات تمرير المحتوى وحدود الطول لا يثبت جودة الصياغة الدلالية.
...
- **روابط المتن أثناء التحرير:** الضغط على الرابط يفتحه في تبويب جديد مع بقاء المحرر مفتوحًا؛ تظهر خيارات تعديل الرابط وإزالته عند تمرير المؤشر عليه.
- **التثبيت والجدولة للمحرر:** إظهار التثبيت يتبع `story.pin` (أو `story.publish` للصلاحيات السابقة)، والموعد يتبع `story.schedule` مستقلًا عن النشر الفوري. الدور `editor` يملك التثبيت والجدولة افتراضيًا؛ الترحيل `0015_editor_pin_schedule` يضيفهما للأدوار المزروعة سابقًا دون تغيير الاستثناءات الفردية أو بقية الصلاحيات. يبقى العاجل والنشر الفوري والأرشفة على صلاحياتها الحالية، والجدولة تمر بالحارس عند الحجز وعند النشر. لا يظهر الموعد للمنشور أو المؤرشف؛ تعديل تثبيت مادة منشورة يتبع مسودة التعديل المعتادة حتى اعتمادها.
...
- **البث بنبضات:** أي أداة تُطلب بترويسة `Accept: application/x-ndjson` تُبثّ من `/api/tahrir/ai/assist` بنبضة كل 10 ثوانٍ وحشو 1100 حرفًا في أول حدث، فلا يقطع الوسيط (Cloudflare/Railway) الاتصال الصامت بـ502 أثناء استدعاءات النموذج الطويلة. القارئ المشترك `lib/ai/read-assist-stream.ts` (`readAssistStream`) تستعمله الملحقات وSEO والعناوين/الموجز والتحرير الشامل، ويعود إلى `readAssistResponse` عند رد JSON عادي أو صفحة وسيط.
