...
import { isCanonicalStoryAliasId } from "./canonical-stories.ts";
...
export async function listSitemapEntries(): Promise<SitemapStoryEntry[]> {
  ...
  for (let offset = 0; ; offset += pageSize) {
    ...
    entries.push(...page.filter((entry) => !isCanonicalStoryAliasId(entry.id)));
...
export async function listVideoSitemapEntries(): Promise<Story[]> {
  ...
  for (let offset = 0; ; offset += pageSize) {
    ...
    entries.push(...page.filter((story) => !isCanonicalStoryAliasId(story.id)));
    if (page.length < pageSize) return entries;
  }
}
