...
export function createNavigationTracker(options: {
  ...
  return {
    start(target: string, from: string, origin: string) {
      ...
      if (url.origin !== origin || !publicPerformanceRoute(previous.pathname)) return false;
      const route = publicPerformanceRoute(url.pathname);
    ...
    commit(pathname: string, search: string) {
      ...
      const route = publicPerformanceRoute(pathname);
