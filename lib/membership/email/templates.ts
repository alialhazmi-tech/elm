...
export function renderAccountEmail(
  ...
  switch (kind) {
    ...
    case "verify-otp":
      ...
      paragraphs = [
        "لتوثيق بريدك الإلكتروني وربطه بحسابك في العلم، أدخل الرمز التالي في صفحة إعدادات الحساب:",
  ...
  const code = otp
    ...
    : "";
  const html = `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${e(subject)}</title></head><body style="margin:0;background:#f7f5f1;font-family:Tahoma,Arial,sans-serif;color:#18232d"><table role="presentation" dir="rtl" width="100%" style="padding:24px 12px"><tr><td align="center"><table role="presentation" width="100%" style="max-width:600px;background:#fff;border:1px solid #e6e1d9;border-radius:18px"><tr><td dir="rtl" style="padding:32px 24px;text-align:right;font-size:16px"><a href="${EMAIL_ORIGIN}" style="display:inline-block;text-decoration:none;background:#204780;padding:14px 18px;border-radius:8px;margin-bottom:24px"><img src="${EMAIL_ORIGIN}/brand/alelm-wordmark.png" width="100" alt="العلم" style="display:block;width:100px;height:auto;border:0"></a><h1 style="font-size:24px;line-height:1.6;margin:0 0 28px">${e(subject)}</h1>${p(greeting)}${p(paragraphs[0])}${code}${kind !== "welcome" ? action : ""}${paragraphs
