...
import { performanceInput } from '@/lib/performance/protocol';
...
export async function POST(request: Request) {
  ...
  const input = performanceInput(await Promise.resolve().then(() => JSON.parse(body)).catch(() => null));
  if (!input) return response(400);
  for (const metric of input) {
    console.info(JSON.stringify({ event: 'web-performance', ...metric,
