...
export default function TahrirError({ retry }: { error: Error & { digest?: string }; retry: () => void }) {
  return (
    <main dir="rtl" className="grid min-h-svh place-items-center bg-background px-6 py-12 text-foreground">
      <div className="w-full max-w-md text-center">
        ...
        <div className="mt-7 flex flex-wrap justify-center gap-3">
          <button type="button" onClick={() => retry()} className="min-h-11 rounded-xl bg-primary px-6 font-semibold text-primary-foreground focus-visible:outline-2 focus-visible:outline-offset-4">إعادة المحاولة</button>
