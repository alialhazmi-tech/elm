...
export default async function MembersPage() {
  ...
  if (!actor?.can("users.view")) return <Forbidden title="الأعضاء" permission="users.view" />;
  ...
  return (
    <main className="flex flex-col gap-3">
      <div className="flex flex-wrap items-baseline gap-3">
        <h1 className="font-display text-xl font-extrabold">الأعضاء</h1>
        <span className="text-xs text-muted-foreground tabular-nums">
          {members.length} عضوًا · {active} فعّال · {members.length - active} معلّق
        </span>
      </div>
      <MembersClient
        members={members}
        roles={roles.map(({ id, label, isSystem }) => ({ id, label, isSystem }))}
