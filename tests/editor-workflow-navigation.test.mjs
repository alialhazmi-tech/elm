...
test('editor publishes updates, withdraws to draft and navigates only after confirmed success', async () => {
  ...
  const mocks = {
    ...
    'lucide-react': 'export const ActivityIcon="svg",ExternalLinkIcon="svg",FilePenLineIcon="svg",HistoryIcon="svg",RefreshCwIcon="svg",SaveIcon="svg",SendIcon="svg",ShieldCheckIcon="svg"',
  ...
  try {
    ...
    await reset('scheduled', false);
    props.canSchedule = true;
    props.canPin = true;
    const editorDetails = nodes(render()).find(node => node.type === 'DetailsPanel');
    assert.equal(editorDetails.props.canApprove, false);
    assert.equal(editorDetails.props.canPin, true);
    assert.equal(editorDetails.props.canSchedule, true);
    editorDetails.props.onPinned(true);
    editorDetails.props.onScheduleAt('2030-01-04T14:00');
    await nodes(render()).find(node => node.type === 'DetailsPanel').props.onSchedule();
    assert.equal(requests.length, 1);
    assert.equal(requests[0].input.pinned, true);
    assert.equal(requests[0].input.updateScheduled, true);
    assert.equal(requests[0].input.rescheduleAt, '2030-01-04T11:00:00.000Z');
    assert.equal(button('اعتماد ونشر'), undefined);
    await reset();
