...
test("الجلسة تحمل الهوية فقط والفاعل يُحلّ من القاعدة مع فحص التعليق", async () => {
  ...
  assert.match(layout, /requireScreenActor\(\)/);
  assert.match(screen, /loadActor\(\)/);
  assert.match(screen, /redirect\("\/tahrir\/password"\)/);
