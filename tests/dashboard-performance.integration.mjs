...
try {
 ...
 for(const [status,filters] of [[undefined,{}],['published',{}],['review',{}],['archived',{}],[undefined,{q:'299',seriesSlug:'absat'}],[undefined,{q:'%_'}]]){
  ...
  assert.deepEqual(nextRows.map(({body,authorId,assignedTo,...row})=>{assert.equal(typeof body,"string");assert.equal(authorId,null);assert.equal(assignedTo,null);return row;}),oldRows);
  for(const row of nextRows) assert.equal(row.body,bodies.get(row.id).body);
  if(oldRows.length) assert.equal(previousQueries,2);
 }
 await client.query("insert into users(id,username,display_name,password_hash,role,created_at) values('list-owner','list-owner','كاتب القائمة','fixture','editor','2026-01-01T00:00:00Z'),('list-assignee','list-assignee','المكلّف','fixture','editor','2026-01-01T00:00:00Z')");
 await client.query("update stories set author_id='list-owner',assigned_to='list-assignee' where id='fixture-30000'");
 const ownershipRow=(await subject.listPageForReview('archived',1,25,{q:'مادة للاختبار 30000'})).find(row=>row.id==='fixture-30000');
 assert.equal(ownershipRow.authorId,'list-owner');assert.equal(ownershipRow.assignedTo,'list-assignee');
