...
try {
  ...
  await build({
    stdin: { contents: `
      ...
      export { invalidateRoleCache, loadActor, canEditStory } from './lib/tahrir/access';
      export { requireScreenActor } from './lib/tahrir/screen';
    ...
    plugins: [{ name: "isolated-matrix", setup(builder) {
      ...
      builder.onResolve({ filter: /^@\/components\/tahrir\/forbidden$/ }, () => ({ path: "forbidden", namespace: "test" }));
      ...
      builder.onLoad({ filter: /.*/, namespace: "test" }, (args) => ({ loader: "js", resolveDir: process.cwd(), contents: ({
        ...
        navigation: "export const redirect=location=>{throw Object.assign(new Error('redirect'),{location})};",
        forbidden: "export const Forbidden=()=>null;",
  ...
  // التعليق والاستئناف والدخول الجديد: لا تُحيى الجلسة الملغاة ولا تضيع صلاحيات أي دور.
  delete process.env.TAHRIR_MFA_ENFORCE;
  for (const role of ["admin", "managing_editor", "editor"]) {
    const id = `resumed-${role}`;
    await admin.query("insert into users(id,username,display_name,password_hash,role,created_at) values($1,$1,$1,$2,$3,$4)", [id, passwordHash, role, at]);
    await admin.query("insert into user_permissions(user_id,permission_key,effect) values($1,'media.upload','deny')", [id]);
    const signIn = async () => {
      const response = await withDb(() => subject.loginApi(json("POST", { username: id, password: "matrix-password-123" })));
      assert.equal(response.status, 200);
      return readSessionToken(response.headers.get("Set-Cookie").split(";")[0].slice(SESSION_COOKIE.length + 1));
    };
    const oldSession = await signIn();
    globalThis.__matrixSession = oldSession;
    const before = await withDb(() => subject.requireScreenActor());
    assert.equal((await call("admin-u", () => subject.memberStatus(json("POST", { status: "suspended", reason: "اختبار الاستئناف" }), ctx(id)))).status, 200);
    globalThis.__matrixSession = oldSession;
    await assert.rejects(withDb(() => subject.requireScreenActor()), error => error.location === "/tahrir/login");
    assert.equal((await withDb(() => subject.loginApi(json("POST", { username: id, password: "matrix-password-123" })))).status, 403);
    assert.equal((await call("admin-u", () => subject.memberStatus(json("POST", { status: "active" }), ctx(id)))).status, 200);
    globalThis.__matrixSession = oldSession;
    await assert.rejects(withDb(() => subject.requireScreenActor()), error => error.location === "/tahrir/login");
    const freshSession = await signIn();
    assert.equal(freshSession.sessionVersion, oldSession.sessionVersion + 1);
    globalThis.__matrixSession = freshSession;
    const after = await withDb(() => subject.requireScreenActor());
    assert.equal(after.role, before.role);
    assert.deepEqual([...after.permissions].sort(), [...before.permissions].sort());
    assert.equal(subject.canEditStory(after, { authorId: id }), true);
    assert.equal(subject.canEditStory(after, { authorId: "another-user" }), role !== "editor");
    assert.equal(subject.canEditStory(after, { authorId: "another-user", assignedTo: id }), true);
    if (role === "editor") {
      assert.equal((await call("admin-u", () => subject.memberOverrides(json("PUT", { overrides: [
        { permissionKey: "media.upload", effect: "deny" },
        { permissionKey: "story.publish", effect: "deny" },
        { permissionKey: "users.manage", effect: "deny" },
        { permissionKey: "story.edit.any", effect: "allow" },
      ] }), ctx(id)))).status, 200);
      globalThis.__matrixSession = freshSession;
      const allowed = await withDb(() => subject.requireScreenActor());
      assert.equal(subject.canEditStory(allowed, { authorId: "another-user" }), true, "the individual edit grant takes effect on the current session");
      assert.equal(allowed.can("story.publish"), false);
      assert.equal(allowed.can("users.manage"), false);
      assert.equal(allowed.can("media.upload"), false);
    }
    // إعادة ضبط الكلمة من الإدارة تتطلب تغييرها، حتى مع layout قديم في المتصفح.
    assert.equal((await call("admin-u", () => subject.memberPassword(json("POST", { password: "temporary-pass-789" }), ctx(id)))).status, 200);
    globalThis.__matrixSession = freshSession;
    await assert.rejects(withDb(() => subject.requireScreenActor()), error => error.location === "/tahrir/login");
    const resetLogin = await withDb(() => subject.loginApi(json("POST", { username: id, password: "temporary-pass-789" })));
    assert.equal(resetLogin.status, 200);
    globalThis.__matrixSession = await readSessionToken(resetLogin.headers.get("Set-Cookie").split(";")[0].slice(SESSION_COOKIE.length + 1));
    await assert.rejects(withDb(() => subject.requireScreenActor()), error => error.location === "/tahrir/password");
  }
  checks++;
