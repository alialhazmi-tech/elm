...
test('metadata requires approval, rejects stale results and copies saved links accurately',async()=>{
  ...
  try{
    ...
    const links={identity:{id:'original-id',section:'sciences',slug:'عنوان-المادة'},published:false,dirty:false};component=()=>ArticleLinks(links);
    ...
    assert.equal(inputs().length,1);assert.equal(copy('نسخ رابط المحرر للزملاء'),undefined);
